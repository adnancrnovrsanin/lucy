...
pipeline actions:
  ...
  --list-scopes                      list .lucy/gates pipelines

targeting:
  --scope ID             use .lucy/gates/ID (or UNLAZY_SCOPE)
...
Approvals live outside the repository under ~/.lucy/approved by default.
...
if (!target.files.length && action !== "--release") {
  failUsage("no gate files found (looked for " + UNLAZY_DIR + "/<scope>/, then GATES.md and *.md directly under " + join(root, UNLAZY_DIR) + ")");
}
...
const approvalDir = resolve(process.env.UNLAZY_APPROVAL_DIR || join(homedir(), ".lucy", "approved"));
