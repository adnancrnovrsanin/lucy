...
export const UNLAZY_DIR = join(".lucy", "gates");
...
function legacyDiscovery(root) {
  // Solo layout: GATES.md and any other Markdown ledger directly under
  // .lucy/gates/. Real subdirectories there are scopes or the lock
  // directory, never solo ledgers.
  const directory = join(root, UNLAZY_DIR);
  if (!namedEntry(directory)) return { files: [], errors: [] };
  try {
    if (!realDirectoryInside(root, directory)) {
    ...
    const files = readdirSync(directory, { withFileTypes: true })
      // Include every named Markdown entry that is not a real directory.
      // Consumers perform the stable-file check, so a FIFO or link cannot
      // disappear as "no gates".
...
export function hookStatePath(root, scope) {
  return scope ? join(scopeRoot(root, scope), "hook-state.json") : join(root, UNLAZY_DIR, "hook-state.json");
}
