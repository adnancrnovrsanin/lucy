...
The current implementation has a separate deterministic test suite for parser, checker, lease, hook, and portability behavior. Run it with the repository's documented test command. Those software tests validate implementation behavior; they do not validate broad claims about model psychology or task productivity.
