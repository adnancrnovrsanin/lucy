...
`CWD:` is resolved relative to the checker's default working directory. Set that default with `--cwd`. Without `--cwd`, explicitly named ledgers anchor beside that ledger, while scoped and solo discovery anchor at `--root`. Keep `CWD:` repository-relative. The resolved directory is part of both evidence and approval.
...
```text
node scripts/gate-lint.mjs .lucy/gates/GATES.md
node scripts/gate-lint.mjs --strict --json .lucy/gates/<scope>/gates/leaf-1.1.1.md
```
...
- [ ] G0: this ledger states outcomes that can fail
  CHECK: node scripts/gate-lint.mjs .lucy/gates/GATES.md
  EXPECT: LINT OK
  EVIDENCE: pending
```
