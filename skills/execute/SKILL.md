...
Execute a plan by dispatching a fresh implementer subagent per task, a task review (spec compliance plus code quality) after each, and a broad whole-branch review at the end. The plan is the file the user passed, usually one under `.lucy/plans/`. When they passed none, ask which plan to run before doing anything else.
...
- A review after each task (spec compliance plus code quality) and a broad review at the end
- Your own context kept free for coordination

## The process

Setup first: worktree, ledger check, read the plan, pre-flight review. Then, per task:

1. Dispatch an implementer subagent from [implementer-prompt.md](implementer-prompt.md). Answer its questions. It implements, tests, commits, and self-reviews.
2. Generate the review package and dispatch a task reviewer from [task-reviewer-prompt.md](task-reviewer-prompt.md).
3. Spec compliant and quality approved: append the completion line to the ledger and mark the todo complete.
...
- Each plan owns a workspace: at skill start, run this skill's `scripts/sdd-workspace PLAN_FILE`. It prints the plan's git-ignored directory (`<repo-root>/.lucy/runs/<plan-basename>/`), home to every artifact for THIS plan: ledger, briefs, reports, review packages. Another plan's directory is never yours to read or write. When the script reports that it added `.lucy/` to the repo's `.gitignore`, commit that one line before dispatching Task 1.
...
- Create the ledger with its identity as the first line: `# execute ledger, plan: <plan file path>`.
...
**Turn count beats token price.** Wall-clock and context cost scale with how many turns a subagent takes, and the cheapest models routinely take two to three times the turns on multi-step work, costing more overall. Use a mid-tier model as the floor for reviewers and for implementers working from prose descriptions. When the task's plan text contains the complete code to write, the implementation is transcription plus testing: use the cheapest tier for that implementer. Single-file mechanical fixes also take the cheapest tier.
...
**Waiting on dispatched subagents:** never poll a wait interface with short timeouts, and never sit in one silent, open-ended wait either. While you have local work, such as ledger updates, packaging the next review, or reading reports, keep working; child results arrive on their own. When you are genuinely idle, wait in bounded stretches (five to ten minutes, where your platform allows), and between stretches post one line of status and reconcile your live children: list them, and chase any that finished without reporting. A bounded stretch keeps nearly all of a long wait's efficiency while guaranteeing a stuck or lost child is noticed within minutes, not at the end of the session.
...
The loop triggers when the review reports spec issues, any Critical or Important finding, or a "cannot verify from diff" item you confirmed as a real gap.
...
Then mark the todo complete and move on. Never move to the next task while the review has open Critical/Important issues that are neither fixed nor parked-with-ruling at the cap, and never while a declared gate ledger is short of ALL MET.
...
| "The implementer's report says the gates are green" | The report is a claim. Only `--reverify` run by you, printing ALL MET, completes a task that declared a ledger. |

## Inline mode

When no subagent tool is available, work the tasks in this session. Setup is the same: worktree, workspace, ledger, plan read, pre-flight scan. Then, one task at a time:

1. Mark the task in progress.
2. Follow each step exactly; the plan has bite-sized steps, and the exact values live in its text.
3. Run the verifications as the plan specifies. Skip none.
4. If the task declares a Gates: ledger, check it through unlazy as in the subagent loop; the task completes only on ALL MET.
5. Append the completion line to the ledger and mark the task complete.
