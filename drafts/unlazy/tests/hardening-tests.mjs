...
test("approval: status is read-only and an unapproved CHECK is printed but not run", async () => {
  ...
  try {
    ...
    s.write(".lucy/gates/GATES.md", gate("G1", "approval", "node check.mjs", "OK"));
    const before = s.read(".lucy/gates/GATES.md");
    ...
    assert(s.read(".lucy/gates/GATES.md") === before, "status changed ledger");
    ...
    s.write(".lucy/gates/GATES.md", "- [x] G1: claimed\n  CHECK: node check.mjs\n  EXPECT: OK\n  EVIDENCE: claimed\n");
...
test("approval: token binds the full oracle and changing CWD invalidates it", async () => {
  ...
  try {
    ...
    s.write(".lucy/gates/GATES.md", gate("G1", "oracle", "node check.mjs", "OK", "  CWD: a\n"));
    ...
    s.write(".lucy/gates/GATES.md", gate("G1", "oracle", "node check.mjs", "OK", "  CWD: b\n"));
...
test("approval: storage failures use the infrastructure exit code", async () => {
  ...
  try {
    ...
    s.write(".lucy/gates/GATES.md", gate("G1", "approval storage", "node ok.mjs", "OK"));
...
test("approval: a lexical outside alias cannot resolve back into the repository", async () => {
  ...
  try {
    ...
    s.write(".lucy/gates/GATES.md", gate("G1", "canonical approval boundary", "node marker.mjs", "OK"));
...
test("approval: an existing store must remain private to its owner", async () => {
  ...
  try {
    s.write("ok.mjs", "console.log('OK');\n");
    s.write(".lucy/gates/GATES.md", gate("G1", "private approval store", "node ok.mjs", "OK"));
...
test("approval: a FIFO record is rejected without blocking or executing", async () => {
  ...
  try {
    s.write("marker.mjs", "import { writeFileSync } from 'node:fs'; writeFileSync('ran.txt','yes'); console.log('OK');\n");
    s.write(".lucy/gates/GATES.md", gate("G1", "regular approval record", "node marker.mjs", "OK"));
    const first = await gateRun(s, []);
    assert(first.code === 0, first.out);
...
test("shell: resolution and PATH context are visible, invalid overrides are usage errors", async () => {
  ...
  try {
    ...
    s.write(".lucy/gates/GATES.md", gate("G1", "shell", "node ok.mjs", "SHELL-OK"));
    const good = await gateRun(s, []);
    assert(good.code === 0, good.out);
    has(good.out, "shell=");
    has(good.out, "PATH=");
    const ledger = s.read(".lucy/gates/GATES.md");
...
test("execution: exit zero and EXPECT must both pass", async () => {
  ...
  try {
    ...
    s.write(".lucy/gates/GATES.md", gate("G1", "nonzero", "node bad.mjs", "MATCH"));
    const result = await gateRun(s, []);
    assert(result.code === 1, result.out);
    has(result.out, "exit=7; EXPECT=matched");
    has(s.read(".lucy/gates/GATES.md"), "- [ ] G1");
  } finally { s.cleanup(); }
});
...
test("execution: failure summaries retain early assertion diagnostics", async () => {
  ...
  try {
    ...
    s.write(".lucy/gates/GATES.md", gate("G1", "diagnostic", "node bad.mjs", "WILL-NOT-MATCH"));
    const result = await gateRun(s, []);
    assert(result.code === 1, result.out);
...
test("evidence: successful output is fingerprinted instead of persisted or echoed", async () => {
  ...
  try {
    ...
    s.write(".lucy/gates/GATES.md", gate("G1", "private output", "node private.mjs", "CHECK_OK"));
    const result = await gateRun(s, []);
...
test("terminal: repository titles and diagnostics cannot emit controls, line separators, or bidi overrides", async () => {
  const s = sandbox();
  try {
    s.write(".lucy/gates/GATES.md", "- [ ] G1: title\u001b]0;owned\u0007\u202eevil\n  EVIDENCE: pending\n");
    ...
    s.write(".lucy/gates/GATES.md", gate("G2", "controlled failure", "node bad.mjs", "NEVER"));
...
test("reverify: a stale success is demoted and a reproducible success stays met", async () => {
  ...
  try {
    ...
    s.write(".lucy/gates/GATES.md", "- [x] G1: forged\n  CHECK: node bad.mjs\n  EXPECT: RIGHT\n  EVIDENCE: forged\n");
    ...
    has(s.read(".lucy/gates/GATES.md"), "- [ ] G1");
    has(s.read(".lucy/gates/GATES.md"), "EVIDENCE: pending");
    s.write("good.mjs", "console.log('RIGHT');\n");
    s.write(".lucy/gates/GATES.md", "- [x] G1: real\n  CHECK: node good.mjs\n  EXPECT: RIGHT\n  EVIDENCE: RIGHT\n");
...
test("parser: fenced examples are ignored and CRLF plus missing EVIDENCE are preserved", async () => {
  ...
  try {
    ...
    const after = s.read(".lucy/gates/GATES.md");
    has(after, "- [x] G1: real\r\n");
...
test("parser: malformed ledgers are usage errors in checker and blocking in hook", async () => {
  ...
  for (const [name, text, expected] of cases) {
    const s = sandbox();
    try {
      s.write(".lucy/gates/GATES.md", text);
      const status = await gateRun(s, ["--status"], { approve: false });
...
test("regex: a decisive regex succeeds and catastrophic matching is bounded", async () => {
  ...
  try {
    ...
    s.write(".lucy/gates/GATES.md", gate("G1", "regex", "node good.mjs", "/total: \\d+ items/"));
    const good = await gateRun(s, []);
    assert(good.code === 0, good.out);
    s.write("evil.mjs", "console.log('a'.repeat(30000) + '!');\n");
    s.write(".lucy/gates/GATES.md", gate("G1", "bounded", "node evil.mjs", "/(a+)+$/"));
...
test("regex: worker startup is outside the match budget and concurrency is capped", async () => {
  ...
  try {
    ...
    s.write(".lucy/gates/GATES.md", ledger);
...
test("execution: output is capped and overflow cannot certify a gate", async () => {
  ...
  try {
    ...
    s.write(".lucy/gates/GATES.md", gate("G1", "bounded output", "node large.mjs", "FINAL"));
    const result = await gateRun(s, []);
...
test("jobs: rolling concurrency is opt-in and output stays in gate order", async () => {
  ...
  try {
    ...
    s.write(".lucy/gates/GATES.md", gate("G1", "one", "node g1.mjs", "OK1") +
...
test("jobs: runner waits for stdio close so delayed descendant output is visible", async () => {
  ...
  try {
    ...
    s.write(".lucy/gates/GATES.md", gate("G1", "late output", "node delayed.mjs", "LATE_OK"));
    const result = await gateRun(s, []);
    assert(result.code === 0, result.out);
    has(result.out, "PASS GATES:G1");
    has(result.out, "output=sha256=");
...
test("writeback: a result cannot certify a gate whose oracle changed in flight", async () => {
  ...
  try {
    ...
    s.write(".lucy/gates/GATES.md", gate("G1", "stale", "node slow.mjs", "OLD"));
    const running = gateRun(s, []);
    await waitForPath(s.path("started.txt"));
    s.write(".lucy/gates/GATES.md", gate("G1", "stale", "node slow.mjs", "NEW"));
...
test("reverify: a stale in-flight result cannot leave old evidence falsely green", async () => {
  ...
  try {
    s.write("slow.mjs", "import { writeFileSync } from 'node:fs'; writeFileSync('started.txt','yes'); setTimeout(()=>console.log('OLD'),800);\n");
    s.write(".lucy/gates/GATES.md", "- [x] G1: stale met\n  CHECK: node slow.mjs\n  EXPECT: OLD\n  EVIDENCE: old evidence\n");
    ...
    assert(s.read(".lucy/gates/GATES.md").includes("EXPECT: NEW\n  EVIDENCE: old evidence"), "newer ledger was clobbered");
  } finally { s.cleanup(); }
});
...
test("CLI: numeric bounds, incompatible modes, and file types fail with exit 2", async () => {
  const s = sandbox();
  try {
    s.write(".lucy/gates/GATES.md", "- [ ] G1: manual\n  EVIDENCE: pending\n");
...
test("targeting: discovered and explicit ledgers reject links and FIFOs without outside reads", async () => {
  ...
  try {
    ...
    mkdirSync(s.path(".lucy/gates"), { recursive: true });
    const top = s.path(".lucy/gates/GATES.md");
    ...
    if (process.platform !== "win32") {
      ...
      rmSync(s.path(".lucy/gates"), { recursive: true, force: true });
      symlinkSync(outsideGates, s.path(".lucy/gates"));
      await assertClosed(["--status"], "symlinked solo gates directory");
      unlinkSync(s.path(".lucy/gates"));

      mkdirSync(s.path(".lucy/gates/scoped"), { recursive: true });
      symlinkSync(outside, s.path(".lucy/gates/scoped/GATES.md"));
      await assertClosed(["--status", "--scope", "scoped"], "symlinked scoped top ledger");
      unlinkSync(s.path(".lucy/gates/scoped/GATES.md"));
    ...
    rmSync(s.path(".lucy/gates"), { recursive: true, force: true });
    s.write(".lucy/gates/GATES.md", "# Gates\n- [ ] C1: LOCAL_CONTROL_2864\n  EVIDENCE: pending\n");
...
test("leases: unsafe declarations, unknown leaves, and wildcard witnesses fail closed", async () => {
  const s = sandbox();
  try {
    s.write(".lucy/gates/a/gates/leaf-a.md", "OWNS: src/a*.js\n\n" + gate("G1", "a", null, null));
    s.write(".lucy/gates/b/gates/leaf-b.md", "OWNS: src/ab*.js\n\n" + gate("G1", "b", null, null));
    ...
    s.write(".lucy/gates/dot-a/gates/leaf.md", "OWNS: src/./same.js\n\n" + gate("G1", "dot a", null, null));
    s.write(".lucy/gates/dot-b/gates/leaf.md", "OWNS: src/same.js\n\n" + gate("G1", "dot b", null, null));
    ...
    s.write(".lucy/gates/prefix-a/gates/leaf.md", "OWNS: src/api\n\n" + gate("G1", "prefix a", null, null));
    s.write(".lucy/gates/prefix-b/gates/leaf.md", "OWNS: src/api/**\n\n" + gate("G1", "prefix b", null, null));
    ...
    s.write(".lucy/gates/c/gates/leaf-c.md", "OWNS: ../outside/**\n\n" + gate("G1", "c", null, null));
...
test("parser: a slash wrapped literal path warns that it became a regex", async () => {
  ...
  try {
    ...
    s.write(".lucy/gates/GATES.md", gate("G1", "config path is reported", "node show.mjs", "/etc/app/conf/"));
    ...
    s.write(".lucy/gates/GATES.md", gate("G2", "typecheck is clean", "node show.mjs", "/Found 0 errors/"));
    ...
    s.write(".lucy/gates/GATES.md", gate("G3", "path pattern", "node show.mjs", "/etc\\/app/"));
...
test("win32 integration: a timed-out shell and nested descendant are both reaped", async () => {
  ...
  try {
    ...
    s.write(".lucy/gates/GATES.md", gate("G1", "nested process times out", "node parent.mjs", "never printed"));
    const started = Date.now();
...
test("execution: timeout settlement is bounded even when a detached descendant keeps pipes", async () => {
  ...
  try {
    ...
    s.write(".lucy/gates/GATES.md", gate("G1", "escaped pipe holder times out", "node escape.mjs", "never printed"));
    const started = Date.now();
    const result = await gateRun(s, ["--timeout", "1"]);
    const elapsed = Date.now() - started;
    assert(result.code === 1, result.out);
...
test("posix integration: an exited shell leader still has its ordinary descendants reaped", async () => {
  ...
  try {
    ...
    s.write(".lucy/gates/GATES.md", gate("G1", "orphaned group member times out", "node exiting-parent.mjs", "never printed"));
    const result = await gateRun(s, ["--timeout", "1"]);
    assert(result.code === 1, result.out);
    has(result.out, "timed out after 1s");
