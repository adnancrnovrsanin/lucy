...
test("validation: line separators are rejected in abandonment reasons", async () => {
  ...
  try {
    ...
    const state = JSON.parse(s.read(".lucy/gates/api/dispatch.json"));
...
test("validation: malformed ids, handles, and state fail closed", async () => {
  ...
  try {
    ...
    s.write(".lucy/gates/api/dispatch.json", "{not json\n");
    result = await run(base("status"), { cwd: s.dir });
...
test("state file: symlinks are rejected without reading or changing the victim", async () => {
  ...
  try {
    ...
    mkdirSync(join(s.dir, ".lucy", "gates", "api"), { recursive: true });
    symlinkSync(join(s.dir, "victim.json"), join(s.dir, ".lucy", "gates", "api", "dispatch.json"));
...
test("state file: a dangling symlink is invalid rather than missing", async () => {
  ...
  try {
    mkdirSync(join(s.dir, ".lucy", "gates", "api"), { recursive: true });
    symlinkSync(join(s.dir, "missing-victim.json"), join(s.dir, ".lucy", "gates", "api", "dispatch.json"));
    const result = await run(base("status"), { cwd: s.dir });
...
test("state file: hard links are rejected without reading or changing the victim", async () => {
  ...
  try {
    s.write("victim.json", EMPTY_DISPATCH_STATE);
    mkdirSync(join(s.dir, ".lucy", "gates", "api"), { recursive: true });
    linkSync(join(s.dir, "victim.json"), join(s.dir, ".lucy", "gates", "api", "dispatch.json"));
    const result = await run(base("status"), { cwd: s.dir });
...
test("state file: oversized input is rejected before JSON parsing", async () => {
  ...
  try {
    ...
    s.write(".lucy/gates/api/dispatch.json", oversized);
    const result = await run(base("status"), { cwd: s.dir });
...
test("state file: replacement during the read is rejected as unstable", async () => {
  ...
  const s = sandbox();
  try {
    const state = join(s.dir, ".lucy", "gates", "api", "dispatch.json");
    const replacement = join(s.dir, ".lucy", "gates", "api", "replacement.json");
    const preload = join(s.dir, "replace-after-lstat.cjs");
    s.write(".lucy/gates/api/dispatch.json", EMPTY_DISPATCH_STATE);
    s.write(".lucy/gates/api/replacement.json", REPLACEMENT_DISPATCH_STATE);
    ...
    assert(s.read(".lucy/gates/api/dispatch.json") === REPLACEMENT_DISPATCH_STATE,
...
test("validation: persisted terminal states must describe a possible lifecycle", async () => {
  ...
  const writeWave = (wave) => s.write(".lucy/gates/api/dispatch.json", JSON.stringify({
    schema: 1,
    waves: { "ready-1": wave },
  }, null, 2) + "\n");
  try {
...
test("hook: invalid dispatch diagnostics cannot inject privileged message lines", async () => {
  ...
  try {
    s.write(".lucy/gates/api/GATES.md", "# Gates\n\n- [x] G1: complete\n  EVIDENCE: checked by test\n");
    const at = "2026-08-24T10:00:00.000Z";
    s.write(".lucy/gates/api/dispatch.json", JSON.stringify({
      schema: 1,
      waves: {
        "ready-1": {
...
test("hook: malformed sibling session entries are discarded without fail-open", async () => {
  ...
  try {
    s.write(".lucy/gates/api/GATES.md", "# Gates\n\n- [ ] G1: unfinished\n  EVIDENCE: pending\n");
    s.write(".lucy/gates/api/hook-state.json", JSON.stringify({
      schema: 1,
      sessions: {
    ...
    const state = JSON.parse(s.read(".lucy/gates/api/hook-state.json"));
...
test("dispatch audit log: a symlink refusal warns after the state commit", async () => {
  ...
  try {
    ...
    mkdirSync(join(s.dir, ".lucy", "gates", "api"), { recursive: true });
    symlinkSync(join(s.dir, "victim.txt"), join(s.dir, ".lucy", "gates", "api", "status.log"));
...
test("dispatch audit log: committed warning sanitizes and bounds hostile path text", async () => {
  ...
  try {
    ...
    mkdirSync(join(root, ".lucy", "gates", "api"), { recursive: true });
    writeFileSync(join(root, "victim.txt"), "safe\n");
    linkSync(join(root, "victim.txt"), join(root, ".lucy", "gates", "api", "status.log"));
    ...
    const state = JSON.parse(readFileSync(join(root, ".lucy", "gates", "api", "dispatch.json"), "utf8"));
...
test("dispatch audit log: every transition survives a poisoned hard-link target", async () => {
  ...
  try {
    s.write("victim.txt", "safe\n");
    mkdirSync(join(s.dir, ".lucy", "gates", "api"), { recursive: true });
    linkSync(join(s.dir, "victim.txt"), join(s.dir, ".lucy", "gates", "api", "status.log"));
    ...
    const beforeInvalid = s.read(".lucy/gates/api/dispatch.json");
    ...
    assert(s.read(".lucy/gates/api/dispatch.json") === beforeInvalid,
...
test("hook: dispatch transitions reset the semantic loop guard but metadata-only edits do not", async () => {
  ...
  try {
    ...
    s.write(".lucy/gates/api/dispatch.json", JSON.stringify(state, null, 2) + "\n");
    for (let index = 0; index < 3; index++) {
      const blocked = await runScript(STOP_HOOK, ["--scope", "api"], { cwd: s.dir, stdin });
      assertHas(blocked.out, '"decision":"block"');
    }
