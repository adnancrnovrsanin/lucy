...
test("locks: real and symlink root aliases serialize one physical target", async () => {
  ...
  try {
    ...
    const realRegistry = s.path(".lucy/gates/locks/lease-registry");
    let announceEntry;
    let releaseHolder;
...
test("leases: a disjoint re-claim cannot overwrite the same live owner", async () => {
  ...
  try {
    ...
    s.write(".lucy/gates/same/gates/leaf.md",
      "OWNS: src/disjoint/**\n# Gates\n- [ ] G1: pending\n  EVIDENCE: pending\n");
...
test("leases: linked and FIFO records fail closed while a regular record remains usable", async () => {
  ...
  try {
    ...
    s.write(".lucy/gates/fresh/gates/leaf.md", ledger);
    const victim = s.write("lease-victim.json", '{"scope":"victim","leaf":"leaf","globs":["src/victim/**"]}\n');
    const poison = s.path(".lucy/gates/locks/poison.lease");
    ...
    const invalid = s.path(".lucy/gates/locks/" + sha256(invalidIdentity).slice(0, 24) + ".lease");
...
test("leases: an explicit release cleans orphaned leases after a scope directory is gone", async () => {
  ...
  try {
    ...
    const lockNames = readdirSync(s.path(".lucy/gates/locks"));
    assert(!lockNames.some((name) => name.endsWith(".lease")), "orphan lease remained: " + lockNames.join(", "));

    s.write(".lucy/gates/live/gates/leaf-live.md", "# malformed zero-gate ledger\n");
...
test("hook: repeated 64-writer bursts are serialized without lost increments", async () => {
  const s = sandbox();
  try {
    s.write(".lucy/gates/GATES.md", "- [ ] G1: pending\n  EVIDENCE: pending\n");
    const payload = JSON.stringify({ cwd: s.dir, session_id: "one-session" });
    for (let round = 1; round <= 3; round++) {
      rmSync(s.path(".lucy/gates/hook-state.json"), { force: true });
      ...
      const state = JSON.parse(s.read(".lucy/gates/hook-state.json"));
...
test("hook: sessions remain isolated and completion/no-gates clears stale state", async () => {
  ...
  try {
    ...
    let state = JSON.parse(s.read(".lucy/gates/hook-state.json"));
...
test("hook: a FIFO session binding is ignored promptly while a regular sibling binding resolves", async () => {
  ...
  try {
    for (const scope of ["a", "b"]) {
      s.write(".lucy/gates/" + scope + "/gates/leaf.md",
        "# Gates\n- [ ] G1: pending\n  EVIDENCE: pending\n");
    }
    const fifo = s.path(".lucy/gates/a/session");
    const made = spawnSync("mkfifo", [fifo], { encoding: "utf8" });
    assert(made.status === 0, "could not create session FIFO: " + made.stderr);
    s.write(".lucy/gates/b/session", "wanted\n");
    ...
    rmSync(fifo);
    s.write(".lucy/gates/a/session", "other\n");
...
test("hook: absent pinned scopes allow while existing unsafe pinned scopes block", async () => {
  ...
  try {
    ...
    symlinkSync(outside.dir, s.path(".lucy/gates/api"));
    ...
    assert(existsSync(s.path(".lucy/gates/hook-state.json")), "invalid scope did not use safe root hook state");
    ...
    try { unlinkSync(s.path(".lucy/gates/api")); } catch { /* absent or unsupported */ }
    s.cleanup();
    outside.cleanup();
  }
});
...
test("hook: named special ledgers fail closed promptly instead of becoming no gates", async () => {
  ...
  try {
    ...
    const assertBlocked = async (label, reason = "gate/ledger/dispatch item(s) need work") => {
      const started = Date.now();
      ...
      has(result.out, reason);
      return result;
    };

    mkdirSync(s.path(".lucy/gates"), { recursive: true });
    const fifo = s.path(".lucy/gates/GATES.md");
    ...
    const directory = await assertBlocked("symlinked gates directory", "must be repaired before Stop");
    assert(!directory.out.includes("OUTSIDE_HOOK_CANARY_5097"),
      "hook traversed the symlinked gates directory\n" + directory.out);
  } finally {
    try { unlinkSync(s.path(".lucy/gates")); } catch { /* absent or unsupported */ }
    s.cleanup();
    outside.cleanup();
  }
});
...
test("hook: linked and FIFO progress state never blocks or changes a victim", async () => {
  ...
  try {
    ...
    const statePath = s.path(".lucy/gates/hook-state.json");
...
test("status log: an existing symlink is refused without touching its target", async () => {
  ...
  const s = sandbox();
  try {
    s.write(".lucy/gates/api/GATES.md", "# Gates\n\n- [ ] G1: pending\n  EVIDENCE: pending\n");
    s.write("victim.txt", "safe\n");
    symlinkSync(s.path("victim.txt"), s.path(".lucy/gates/api/status.log"));
...
test("status log: an existing hard link is refused without touching its sibling", async () => {
  ...
  try {
    s.write(".lucy/gates/api/GATES.md", "# Gates\n\n- [ ] G1: pending\n  EVIDENCE: pending\n");
    s.write("victim.txt", "safe\n");
    linkSync(s.path("victim.txt"), s.path(".lucy/gates/api/status.log"));
    const result = await run(GATE_CHECK, ["--scope", "api", "--log", "attacker-controlled append"], { cwd: s.dir });
...
test("status log: a FIFO is rejected without blocking the logger", async () => {
  ...
  try {
    s.write(".lucy/gates/api/GATES.md", "# Gates\n\n- [ ] G1: pending\n  EVIDENCE: pending\n");
    const fifo = s.path(".lucy/gates/api/status.log");
    const made = spawnSync("mkfifo", [fifo], { encoding: "utf8" });
    assert(made.status === 0, "could not create FIFO fixture: " + made.stderr);
    const started = Date.now();
