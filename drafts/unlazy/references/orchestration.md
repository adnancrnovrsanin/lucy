...
5. **Verify each return independently.** Record the native return in its wave, then re-run the returned leaf's runnable gates, including already checked gates:
   ...
   ```text
   node <skill-dir>/scripts/gate-check.mjs --root . --cwd . --reverify .lucy/gates/<scope>/gates/leaf-1.2.1.md
   ```
...
```text
node <skill-dir>/scripts/gate-check.mjs --root . --cwd . --reverify --jobs 4 .lucy/gates/<scope>/gates/leaf-1.1.1.md .lucy/gates/<scope>/gates/leaf-1.1.2.md
```
...
4. **Stop hook.** Lucy's Stop hook blocks the driver from ending while its resolved pipeline has unmet ledgers or incomplete dispatch waves. It does not execute checks or validate their meaning.
