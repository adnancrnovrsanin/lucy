...
`.lucy/` is git-ignored by Lucy. Solo work uses `GATES.md`, or any other `*.md` ledger, directly under `.lucy/gates/`; the subdirectories are scopes.
...
4. the solo ledgers directly under `.lucy/gates/` when no scoped pipeline exists
...
The hook may be pinned with a `--scope` argument in its registered command or resolve a session binding written by:
